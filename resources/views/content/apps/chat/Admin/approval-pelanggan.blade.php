    ...
    if (searchInput) {
        searchInput.addEventListener('input', function() {
            ...
            document.querySelectorAll('[data-searchable]').forEach(el => {
                el.style.display = el.dataset.searchable.includes(val) ? '' : 'none';
            });
        });
    }
    ...
    <div class="main-card">
        ...
        <div class="toolbar">
            <div class="search-box">
                ...
                <input type="text" id="clientSearch" placeholder="Cari di tabel...">
            </div>
            <form action="{{ route('admin.marketing.approval') }}" method="GET" class="d-flex gap-2" style="margin-left:auto">
                <input type="hidden" name="status" value="{{ $statusFilter }}">
                <div class="search-box">
                    <i class="ri-filter-3-line search-icon"></i>
                    <input type="search" name="search" placeholder="Filter..." value="{{ $search }}">
                </div>
                <button type="submit" class="btn-sh btn-sh-primary"><i class="ri-search-line"></i></button>
            </form>
        </div>
