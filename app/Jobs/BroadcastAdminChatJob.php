...
class BroadcastAdminChatJob implements ShouldQueue
{
    ...
    public function __construct(
        ...
        // Parse boolean env safely (handles "false", "0", etc).
        $this->sendPush = filter_var(env('BROADCAST_PUSH_ENABLED', false), FILTER_VALIDATE_BOOLEAN);
    }
    ...
    public function handle(): void
    {
        ...
        $done = 0;
        $sendPush = filter_var(env('BROADCAST_PUSH_ENABLED', $this->sendPush), FILTER_VALIDATE_BOOLEAN);
        ...
        try {
            Pelanggan::select('id', 'webpushr_sid')->chunkById(200, function ($pelanggans) use (&$done, $cacheKey, $sendPush) {
                foreach ($pelanggans as $pelanggan) {
                    ...
                    if ($sendPush && $pelanggan->webpushr_sid) {
                        try {
                            ...
                            // Lewati rate limit / error push, jangan hentikan loop
                            Log::warning('Broadcast push skipped', [
